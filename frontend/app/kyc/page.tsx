...
export default function KYCPage() {
  ...
  const { isValid: isOnChainVerified, refetch: refetchKYC } = useKYCVerified(wallet);
  ...
  const handleSimulateWhitelist = () => {
    if (!wallet) return;
    writeContract({
      address: IDENTITY_REGISTRY,
      abi: IDENTITY_REGISTRY_ABI,
      functionName: 'registerIdentity',
      args: [wallet, wallet, 826], // UK country code
    });
  };
      ...
      <div style={{ background: 'var(--white)', borderBottom: '1px solid var(--border)', padding: '2rem 0' }}>
        <div className="container">
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: 14 }}>
            Required by law before you can invest. Your verification status is stored securely
            on-chain — protecting your identity.
          </p>
        </div>
      </div>
      ...
      <div className="container" style={{ padding: '2rem 1.25rem', maxWidth: 660, margin: '0 auto' }}>
        ...
        {isConnected && effectiveStep === 'intro' && (
          <div className="card" style={{ textAlign: 'center', padding: '2.5rem' }}>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: 14, lineHeight: 1.7, marginBottom: 24 }}>
              ...
              regulations. Your KYC status is verified on-chain via ERC-3643 identity registries.
              This process takes under 5 minutes.
            </p>
            <div className="grid-3" style={{ gap: '0.75rem', marginBottom: 24 }}>
              {[
                { icon: ShieldCheck, label: 'Verified on-chain' },
            ...
            {effectiveStep === 'selfie' && (
              <div className="card" style={{ padding: '1.5rem' }}>
                ...
                <p style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 8, textAlign: 'center' }}>
                  Your KYC data is submitted securely. Once verified, your status will be updated in the <code>IdentityRegistry</code> on Arbitrum Sepolia.
                </p>
              </div>
            )}
          </>
        ...
        {isConnected && effectiveStep === 'pending' && (
          <div className="card" style={{ padding: '2.5rem', textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: 14, lineHeight: 1.7, marginBottom: 24 }}>
              ...
              verification within <strong>1–2 business hours</strong>. Your status
              will be updated in the <code>IdentityRegistry</code> once approved.
            </p>
            ...
            {/* Arbiscan link */}
            <a
              href={`https://sepolia.arbiscan.io/address/0xE413130d308d1587d0E080E9FFB4bb2826952701`}
              target="_blank"
              rel="noopener noreferrer"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--brand)', marginBottom: 20 }}
            >
              View IdentityRegistry on Arbiscan <ExternalLink size={13} />
            </a>

            {/* Sandbox Whitelisting Sim Tool */}
            <div style={{ background: 'var(--brand-light)', borderRadius: 8, padding: '1.25rem', border: '1px solid rgba(26,111,168,0.15)', textAlign: 'left' }}>
              <p style={{ fontWeight: 700, fontSize: 13, color: 'var(--brand)', marginBottom: 6 }}>Sandbox Testing Tool</p>
              <p style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 12, lineHeight: 1.5 }}>
                As this is a sandbox demo, you can instantly whitelist this wallet address in the <code>IdentityRegistry</code> on-chain by clicking the button below. You must execute this transaction using the Deployer account (or any IdentityRegistry agent wallet).
              </p>
              <button
                className="btn btn-primary btn-sm"
                onClick={handleSimulateWhitelist}
                disabled={busy}
              >
                {busy ? 'Registering on-chain…' : 'Simulate KYC Approval (On-Chain)'}
              </button>
            </div>
          </div>
        )}
        ...
        {isConnected && effectiveStep === 'approved' && (
          <div className="card" style={{ padding: '2.5rem', textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: 14, lineHeight: 1.7, marginBottom: 24 }}>
              Your wallet is now whitelisted on-chain in the
              <code>IdentityRegistry</code> on Arbitrum Sepolia.
            </p>
            <div style={{ background: 'var(--green-bg)', borderRadius: 8, padding: '0.75rem', marginBottom: 24, fontSize: 13, color: 'var(--green)', fontWeight: 600 }}>
              ✓ Wallet registered · ERC-3643 Compliance Active · KYC Approved
            </div>
            <a href="/dashboard">
