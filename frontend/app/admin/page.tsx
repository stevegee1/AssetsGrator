    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      {validAddr && (
        <>
          ...
          <div className="card" style={{ padding: '1.25rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
              {[
                ...
                { label: 'Annual Yield',   value: annual    !== undefined ? `£${Number(formatUnits(annual, 6)).toLocaleString()}` : '…' },
          ...
          {/* Deposit rent */}
          <div className="card" style={{ padding: '1.25rem' }}>
            <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 8 }}>Deposit Revenue (GBP)</h3>
            <p style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 8 }}>
              Approve GBP first, then deposit. Yield payments go into the vault for distribution.
            </p>
            <div style={{ display: 'flex', gap: 8 }}>
              <input id="rent-amount" type="number" min="1" step="1" placeholder="GBP amount (e.g. 4500)"
                value={rentAmt} onChange={e => setRentAmt(e.target.value)} style={{ flex: 1 }} />
              <button id="approve-gbp-btn" className="btn btn-outline" disabled={busy || !rentAmt}
          ...
          <div className="card" style={{ padding: '1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <h3 style={{ fontSize: 14, fontWeight: 700 }}>Distribute Yield</h3>
              <p style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 2 }}>
                Pays out pending GBP proportionally to all token holders
              </p>
            </div>
          ...
          <div className="card" style={{ padding: '1.25rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              <input id="appraisal-val" type="number" min="1" placeholder="New value in GBP (e.g. 550000)"
                value={appraisalVal} onChange={e => setAppraisalVal(e.target.value)} />
              <input id="appraisal-note" placeholder="Notes (e.g. Q1 2026 appraisal)"
