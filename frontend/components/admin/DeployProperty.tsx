...
type TokenForm = {
  ...
  pricePerUnit: string;  // GBP per unit, 18 decimals on-chain
  valuationUSD: string;  // total property value in GBP, 18 decimals on-chain
};
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <div className="card" style={{ padding: '1.25rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
          ...
          <div>
            <label>Price Per Unit (GBP) <span style={{ color: 'var(--red)' }}>*</span></label>
            <input id="tok-price" type="number" step="0.01" value={form.pricePerUnit} onChange={set('pricePerUnit')} />
          </div>
          <div>
            <label>Asset Valuation (GBP) <span style={{ color: 'var(--red)' }}>*</span></label>
